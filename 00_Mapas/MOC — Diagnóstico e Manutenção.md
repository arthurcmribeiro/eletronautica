---
title: "MOC — Diagnóstico e Manutenção"
note_type: "moc"
domain: "00_Mapas"
status: "moc-curated"
fase_6_reescrita: 116
reviewed_on: "2026-04-26"
review_jurisdiction: "Brasil"
review_level: "moc-curated"
normas_citadas: []
aliases:
  - "Hub de Diagnóstico e Manutenção"
seo_title: "MOC de diagnóstico e manutenção da elétrica náutica"
seo_description: "Hub transversal para inspeção, medição, sintomas, manutenção preventiva e diagnóstico de falhas em elétrica náutica."
geo_queries:
  - "Como diagnosticar falhas elétricas em embarcações?"
  - "Quais notas usar para manutenção e troubleshooting na elétrica náutica?"
related_notes:
  - "Fundamentos da Elétrica Náutica"
  - "MOC — Segurança Integrada"
  - "Referência Rápida — Valores de Campo"
---

# MOC — Diagnóstico e Manutenção

> [!abstract] Resumo técnico
> Este hub transversal organiza o material útil para inspeção, medição, troubleshooting e manutenção preventiva, independentemente do domínio em que a nota técnica reside.

## Instrumentação e medição

- [[Multímetro e Instrumentos de Medição]]
- [[Voltímetro - Amperímetro (DC e AC)]]
- [[Ferramentas do Eletricista Náutico]]
- [[Referência Rápida — Valores de Campo]]

## Diagnóstico de falhas

- [[Troubleshooting — Diagnóstico de Falhas Elétricas]]
- [[Inspeção de Cabos Terminais e Conexões]]
- [[Correntes Parasitas — Stray Currents]]
- [[Eletrólise]]

## Manutenção preventiva

- [[Manutenção Preventiva Elétrica — Checklist]]
- [[Aterramento]]
- [[Fusíveis DC — Proteção Contra Sobrecorrente]]
- [[Disjuntores (DC) e (AC)]]
- [[Terminais Conectores e Emendas]]

## Diagnóstico por família de sistema

### Energia e baterias

- [[Bancos de Bateria]]
- [[Monitor de Bateria - BMV - Shunt]]
- [[BMS — Battery Management System]]
- [[Carregador de Bateria (AC To DC)]]

### AC, proteção e distribuição

- [[CAIS (Pier) (AC)]]
- [[Gerador (AC)]]
- [[Inversora (DC To AC)]]
- [[Proteção Dr]]
- [[Isolador Galvânico - Transformador de Isolamento]]

### Utilidades e segurança

- [[Bomba de Porão]]
- [[Ar-Condicionado Marine — Sistema Completo]]
- [[Sistema de Alarme Geral - Painel de Alarmes]]
- [[VHF]]

## Perguntas que este mapa ajuda a responder

- Por onde começo quando o sintoma é difuso?
- Quais medições devo fazer antes de trocar componente?
- Em quais notas procurar inspeção, causa raiz e manutenção preventiva?
