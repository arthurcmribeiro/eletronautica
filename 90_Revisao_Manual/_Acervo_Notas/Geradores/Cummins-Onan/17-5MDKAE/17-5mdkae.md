---
title: "Cummins Onan 5MDKAE, A-1356c - Referencia tecnica - 500-3017"
note_type: "acervo-companion"
domain: "90_Revisao_Manual"
status: "auto-extracted"
acervo_origin: "acervo-principal"
document_kind: "technical-reference"
curation_priority: "alta"
curation_stage: "triagem-automatica"
reviewed_on: "2026-04-28"
review_jurisdiction: "Brasil"
source_pdf: "Geradores/Cummins-Onan/17-5MDKAE/17-5mdkae.pdf"
source_sha256: "448b5df582c7014ec6b5bfc2fa0a87df5fd322dd3740fc48def9bc72226032ca"
acervo_system: "Geradores"
acervo_brand: "Cummins-Onan"
acervo_family: "17-5MDKAE"
extraction_method: "pdftotext"
text_extractable: "sim"
pdf_pages: "4"
pdf_version: "1.3"
pdf_encrypted: "nao"
qpdf_status: "warning"
ocr_priority: "nao"
ocr_status: "nao-gerado"
ocr_text_path: "n/a"
ocr_note_path: "n/a"
ocr_text_chars: "0"
analysis_source: "pdftotext"
detected_document_codes: "500-3017, SAEJ1349, 574-5000, 574-5298"
curation_topics: "alimentacao eletrica, protecao e seccionamento, cabos e ligacoes, aterramento/bonding, instalacao fisica, operacao, manutencao, diagnostico de falhas"
aliases:
  - "17-5mdkae.pdf"
related_notes:
  - "90_Revisao_Manual/10_Indices_e_Paineis/Acervo Local - Indice Gerado"
  - "Tabela-Mestre do Acervo - Biblioteca de Referencia"
  - "90_Revisao_Manual/10_Indices_e_Paineis/Acervo Notas - Indice Gerado"
---

# Cummins Onan 5MDKAE, A-1356c - Referencia tecnica - 500-3017

> [!abstract] Resumo tecnico
> Nota companheira gerada automaticamente a partir do PDF do acervo local.
> Ela serve como camada de rastreio rapido, busca textual e preparo de curadoria editorial.

## Fonte

- PDF: [17-5mdkae.pdf](</C:/Users/User/Desktop/ELETRO NAUTICA OBSIDIAN/ELETRO NAUTICA/90_Revisao_Manual/_Acervo_Local/Geradores/Cummins-Onan/17-5MDKAE/17-5mdkae.pdf>)
- caminho relativo: `Geradores/Cummins-Onan/17-5MDKAE/17-5mdkae.pdf`
- nota companheira: `90_Revisao_Manual/_Acervo_Notas/Geradores/Cummins-Onan/17-5MDKAE/17-5mdkae.md`
- tamanho do arquivo: `363.0 KB`
- paginas detectadas: `4`
- versao PDF: `1.3`
- PDF criptografado: `nao`
- titulo em metadados: `n/a`
- texto extraivel detectado: `sim`
- tipo documental detectado: `technical-reference`
- metodo de extracao: `pdftotext`
- status qpdf: `warning`
- prioridade OCR: `nao`
- status OCR auxiliar: `nao-gerado`
- nota OCR auxiliar: `n/a`
- texto OCR auxiliar: `n/a`
- caracteres OCR auxiliares: `0`
- fonte de analise automatica: `pdftotext`

## Sinais principais

- sistema: `Geradores`
- marca: `Cummins-Onan`
- familia: `17-5MDKAE`
- marcas detectadas: `Cummins / Onan`
- codigos/modelos detectados: `5MDKAE, A-1356c`
- codigos tecnicos/documentais detectados: `500-3017, SAEJ1349, 574-5000, 574-5298`
- datas/revisoes detectadas: `n/a`
- termos uteis detectados: `air, alternator, battery, control, cooling, corrosion, engine, generator`
- topicos de curadoria: `alimentacao eletrica, protecao e seccionamento, cabos e ligacoes, aterramento/bonding, instalacao fisica, operacao, manutencao, diagnostico de falhas`

## Extracao tecnica automatica

### Secoes e assuntos provaveis

- `Specifications subject to change without notice`
- `12 V DC electric system with battery`
- `DC control console mounted with isolation`
- `Freshwater cooling system with heat`
- `installation`
- `remote control`
- `Keel cooling with wet exhaust`
- `Keel cooling with dry exhaust, up or down`
- `• 24 V DC electric system with battery`
- `• 24 V electric system with battery charging,`
- `operation of remote panel gauges and set`
- `Battery`
- `Low coolant level safety shut-off switch`
- `• Cruise kit - maintenance parts - fuel filters,`

### Especificacoes e linhas de valor tecnico

- `12 V DC electric system with battery`
- `• 24 V DC electric system with battery`
- `3 phase, 125 C broad range alternator`
- `• Switchable between 50/60 Hz`
- `bolt mounting; (12 or 24 V DC clutch)`
- `bolt mounting; (12 or 24 V DC clutch),`
- `60 Hz: 14.0 Gal/min (53.0 L/min)`
- `50 Hz: 10.0 Gal/min (37.9 L/min)`
- `60 Hz: 9.0 Gal/min (34.1 L/min)`
- `50 Hz: 7.0 Gal/min (27 L/min)`
- `0 F (-18 C) minimum; 24 V optional`
- `21.5 kW 60 Hz`
- `17.5 kW 50 Hz`
- `• 24 V electric system with battery charging,`

### Leitura operacional sugerida

- alimentacao eletrica
- protecao e seccionamento
- cabos e ligacoes
- aterramento/bonding
- instalacao fisica
- operacao
- manutencao
- diagnostico de falhas

## Aplicacao e integracoes sugeridas

- usar esta nota para localizar rapidamente procedimentos, identificacao de modelo e pontos de manutencao do grupo gerador
- prioridade alta para campo: documentar ligacoes, bitolas, protecoes, ventilacao, posicionamento e checklist de montagem

## Links internos sugeridos

- `Tabela-Mestre do Acervo - Biblioteca de Referencia`
- `90_Revisao_Manual/10_Indices_e_Paineis/Acervo Local - Indice Gerado`
- `90_Revisao_Manual/00_Entrada_e_Controle/Portal do Acervo - Biblioteca de Referencia`
- `90_Revisao_Manual/90_Arquivo_Historico/Acervo Local - Geradores Iniciais (Cummins Onan)`

<!-- CURADORIA-HUMANA:START -->
## Curadoria humana

### Resumo humano
- pendente

### Aplicacao de oficina
- pendente

### Modelos cobertos confirmados
- `5MDKAE`

### Pontos de atencao
- pendente

### Integracoes e links internos
- sistema-base: `Geradores`
- marca/familia: `Cummins-Onan / 17-5MDKAE`
- notas relacionadas: pendente

### Status de curadoria
- tipo documental confirmado: `technical-reference`
- curadoria humana: pendente
- pronto para ensino/SEO: nao
<!-- CURADORIA-HUMANA:END -->

## Trechos indexaveis

- `12 V DC electric system with battery`
- `• 24 V DC electric system with battery`
- `3 phase, 125 C broad range alternator`
- `• Switchable between 50/60 Hz`
- `bolt mounting; (12 or 24 V DC clutch)`
- `bolt mounting; (12 or 24 V DC clutch),`
- `60 Hz: 14.0 Gal/min (53.0 L/min)`
- `50 Hz: 10.0 Gal/min (37.9 L/min)`
- `60 Hz: 9.0 Gal/min (34.1 L/min)`
- `50 Hz: 7.0 Gal/min (27 L/min)`
- `0 F (-18 C) minimum; 24 V optional`
- `21.5 kW 60 Hz`

## Perguntas de curadoria

- quais modelos e revisoes este PDF cobre de verdade, e quais ficaram de fora?
- quais paginas merecem virar nota humana curada ou material de aula?
- quais requisitos de montagem, bitola, protecao e ambiente precisam ser destacados no resumo humano?

## Acoes sugeridas

- destacar ligacoes, torque, bitolas, protecoes e condicoes de montagem
- conferir se existe manual operacional complementar na mesma familia

## Observacoes

- esta nota e automatica; revisar manualmente antes de usar como resumo definitivo
- quando este PDF for relevante para ensino, oficina ou SEO, vale evoluir para nota humana curada
