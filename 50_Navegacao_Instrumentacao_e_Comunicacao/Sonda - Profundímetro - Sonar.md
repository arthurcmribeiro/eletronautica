...
<!-- TODO-CITAÇÃO: o padrão ABYC específico para thruhull fittings está sob verificação externa. Versão anterior rotulava como "ABYC A-28 — thruhull fittings"; na nota mestre A-28 aparece como "galvanic isolators" e no registro central consta como `desconhecido` — atribuição a "thruhull fittings" não confirmada. Referência removida até verificação (ver _Editorial/registro_normas.yaml). -->
- **ABYC (padrão específico para thruhull/transdutores a verificar)** — em literatura marinha costuma-se citar a série H para thruhull fittings (ex.: H-27), mas confirmação externa é necessária antes de afirmar o código.
